...
- **Windows用実行ファイル（.exe）のビルド**:
  ...
  npm run build:win
  ```
  - `release/mace-man 3D Builder Setup 1.1.3.exe`（NSISインストーラー）
  - `release/mace-man 3D Builder 1.1.3.exe`（インストール不要のポータブル実行ファイル）
...
`.github/workflows/build.yml` を同梱しているため、リポジトリをプッシュまたは `v1.1.3` などのタグを作成するだけで、GitHub上の Windows / macOS / Linux ランナーが並列で実行ファイルを自動ビルドし、GitHub Releases へ即座に配布バイナリをアップロードします。
